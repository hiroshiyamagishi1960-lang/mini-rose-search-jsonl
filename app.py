# app.py — v5.3.3-stable（UI/環境変数は不変更｜検索v5.3.2維持｜“起動ゲート”で安定駆動）
# 目的：
#  - 既存UI/環境変数を変えずに安定運用（外部取得の成否に関わらず“壊れた版”は公開されない）
...
# ====== 起動ゲート：KB未読込なら起動を失敗させる（＝旧版維持で安定） ======
@app.on_event("startup")
def _startup():
    # 1) まずファイル同梱のkb.jsonlを読み込む（存在すればOK）
    _refresh_kb_globals(fetch_now=False)
    # 2) 無ければバックグラウンドで取得を試す（即応性）
    if not _KB_ROWS and KB_URL and requests is not None:
        th = threading.Thread(target=_bg_fetch_kb, daemon=True)
        th.start()
    # 3) 最終チェック：少しだけ待っても空なら“起動失敗”にする
    #    → Renderはこのデプロイを採用せず、公開中の旧安定版を維持
    for _ in range(10):
        if _KB_ROWS:
            break
...
def build_item(
    ...
    """検索結果1件分の表示用データを作る（タイトル＋スニペット）。

    - 1件目だけ：本文先頭300文字
    - 2件目以降：検索語の周辺を基本160文字前後
    - ただし、「タイトル：〜」行があれば必ず行全体を含める（途中で切らない）
    """
    ...
    if is_first_in_page:
        ...
        if pos < 0:
            start = 0
            end = min(len(body), 160)
        else:
            start = max(0, pos - 80)
            end = min(len(body), pos + 80)

        # ------------------------------------------------
        # ② 「タイトル：」行がある場合は必ず丸ごと含める
        # ------------------------------------------------
        tls = body.find("タイトル：")  # タイトル行の先頭位置
        if tls >= 0:
            # 行末（改行）までをタイトル行とみなす
            nl = body.find("\n", tls)
            if nl >= 0:
                ...
                # 改行が無い場合は適当に少し先まで
                tle = min(len(body), tls + 50)

            # いまの [start, end) でタイトル行を全部含んでいなければ調整
            if not (start <= tls and tle <= end):
                new_start = min(start, max(0, tls - 10))
                new_end = max(end, min(len(body), tle + 10))

                # 長くなりすぎた場合は、タイトル行を守りながら最大220文字程度に抑える
                max_len = 220
                if new_end - new_start > max_len:
                    over = (new_end - new_start) - max_len
                    # タイトル行の前後どちら側をどれくらい削れるか計算
                    before = max(0, tls - new_start)
                    after = max(0, new_end - tle)

                    # 左側（タイトルの前）を少し削る
                    shrink_left = min(over // 2, max(0, before - 5))
                    new_start += shrink_left
                    over -= shrink_left

                    # 右側（タイトルの後ろ）も必要なら削る
                    if over > 0:
                        shrink_right = min(over, max(0, after - 5))
                        new_end -= shrink_right

                start, end = new_start, new_end

        # 最終的なスニペットに「…」を付ける（途中抜粋の場合）
        core = body[start:end]
        snippet_src = ("…" if start > 0 else "") + core + ("…" if end < len(body) else "")
...
def api_search(
    ...
    try:
        ...
        payload = {
            "items": items, "total_hits": total, "page": page, "page_size": page_size,
        ...
        return json_utf8({"items": [], "total_hits": 0, "page": 1, "page_size": page_size,
                          "has_more": False, "next_page": None, "error": "exception", "message": textify(e)}, status=500)
