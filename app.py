# app.py — Mini Rose Search API（JSONL→直読み版互換の正規化＋合成ヘッダ／抜粋500字）
# 版: jsonl-2025-10-17-directlike
# 目的:
#  - JSONLの素データを「直読み版」と同じ論理構造(title/issue/date/author/text/section/url, header)へ“読み込み時に正規化”
#  - 合成ヘッダ header = 「第NN号 | YYYY | title_base」
#  - スコア: header/title > section > text、フレーズ/近接強化、"コンテスト結果"軽ブースト
#  - 並び: score desc → date desc（ページング前に確定）
#  - 抜粋は maxlen=500
#  - /export_normalized で正規化後JSONLを出力（必要時に派生JSONLを生成可能）
...
app = FastAPI(title="Mini Rose Search API", version="jsonl-2025-10-17-directlike")
...
def _best_date(rec_norm:Dict[str,Any])->dt.date:
    # 年は header/text からも拾う
    yset=set(_years_from_text(rec_norm.get("date",""))+_years_from_text(rec_norm.get("text",""))+_years_from_text(rec_norm.get("title",""))+_years_from_text(rec_norm.get("header","")))
...
def _normalize_record(rec:Dict[str,Any])->Dict[str,Any]:
    ...
    header = " | ".join([p for p in [issue, year_str, title_base] if p])

    # 直読み版に合わせた論理構造
    out = {
        "title": title_base,
        "issue": issue,        # 例: "第12号"
        "date": date,          # 例: "2025-10-01" / "2025年10月"
        "author": author,
        "section": section,
        "text": _nfkc(text),
        "url": url,
...
def _record_text_all_norm(rec:Dict[str,Any])->str:
    parts=[rec.get("header",""), rec.get("title",""), rec.get("section",""), rec.get("author",""), rec.get("issue",""), rec.get("date",""), rec.get("text","")]
...
# ---- スコアリング（直読み版寄せ）----
W_HEAD, W_TITLE, W_SECT, W_TEXT = 2.6, 2.2, 1.3, 1.2
PHRASE_HEAD_BONUS, PHRASE_TITLE_BONUS, PHRASE_TEXT_BONUS = 4.5, 4.0, 2.2
COOCCUR_WINDOW = 30
...
def _score(rec:Dict[str,Any], terms_all:List[str], base_terms:List[str], q_raw:str)->Tuple[float, dt.date]:
    ...
    # 専用ブースト
    if ("コンテスト結果" in head) or ("コンテスト結果" in title):
        sc += 1.8

    # 日付キー
    d=_best_date(rec)
    return (sc if matched else 0.0, d)
...
def health():
    ...
    return JSONResponse({"ok":True,"kb_url":KB_URL,"kb_size_raw":len(RAW_DATA),"kb_size_norm":len(NORM_DATA),"kb_fingerprint":sha.hexdigest()}, headers=JSON_HEADERS)

@app.get("/version")
def version():
    return JSONResponse({"version": app.version}, headers=JSON_HEADERS)

@app.get("/export_normalized", response_class=PlainTextResponse)
def export_normalized():
    # 正規化後のJSONLをテキストで返す（派生JSONLを作りたい場合に使用）
    lines=[]
    for r in NORM_DATA:
        lines.append(json.dumps(r, ensure_ascii=False))
    txt="\n".join(lines)
    return PlainTextResponse(txt, headers={"content-type":"text/plain; charset=utf-8","Cache-Control":"no-store"})
...
def api_search(
    ...
    try:
        ...
        for i,(score, d, r) in enumerate(slice_):
            head  = _nfkc(r.get("header",""))
            title = _nfkc(r.get("title",""))  # UIはtitleをそのまま表示する前提
            text  = _nfkc(r.get("text",""))
            title_pos=_find_first(head or title, hl_keys)
            body_pos =_find_first(text, hl_keys)
            ...
            items.append({
                "title": (head or title) + (tag or ""),
                "content": snippet,
                "url": _nfkc(r.get("url","")),
                "source": _nfkc(r.get("url","")),
                "rank": start+i+1
            })
